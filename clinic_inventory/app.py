...
def add_item():
    ...
    if request.method == 'POST':
        ...
        # Add minimum_supply (could be from form or default value)
        minimum_supply = request.form.get('minimum_supply', 0)  # Default to 0 if not provided
        
        # Insert the data into the database, including the photo URL
        query_db("""
            INSERT INTO items 
            (name, quantity, location, category, description, photo_url, instrument_type, expiration_date, 
             supplier, dosage, form, lot_number, ndc, dispense_used, unit_quantity, prescription_type, minimum_supply)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, [name, quantity, location, category, description, photo_url, instrument_type, expiration_date, 
              supplier, dosage, form, lot_number, ndc, dispense_used, unit_quantity, prescription_type, minimum_supply])
...
def transfer_item(item_id):
    ...
    # Check for an identical item in the new location
    existing_item = query_db("""
        SELECT * FROM items WHERE 
            name = ? AND category = ? AND location = ? AND expiration_date = ? AND supplier = ? 
            AND dosage = ? AND form = ? AND lot_number = ? AND ndc = ? AND dispense_used = ? 
            AND unit_quantity = ? AND prescription_type = ? AND instrument_type = ?
    """, [
        item['name'], item['category'], new_location, item['expiration_date'], item['supplier'], 
        item['dosage'], item['form'], item['lot_number'], item['ndc'], item['dispense_used'], 
        item['unit_quantity'], item['prescription_type'], item['instrument_type']
    ], one=True)

    if existing_item:
        # Update quantity of the existing item
        query_db("UPDATE items SET quantity = quantity + ? WHERE id = ?", [quantity_to_transfer, existing_item['id']])
        flash(f"Added {quantity_to_transfer} to existing item in {new_location}", 'success')
    else:
        # Insert a new record if no identical item exists
        new_item = {
            'name': item['name'],
            ...
            'ndc': item['ndc'],
            'dispense_used': item['dispense_used'],
        ...
        query_db("""
            INSERT INTO items (name, category, location, quantity, expiration_date, supplier, dosage, form, lot_number, ndc,
                               dispense_used, unit_quantity, prescription_type, instrument_type, minimum_supply, photo_url, description)
            VALUES (:name, :category, :location, :quantity, :expiration_date, :supplier, :dosage, :form, :lot_number, :ndc,
                    :dispense_used, :unit_quantity, :prescription_type, :instrument_type, :minimum_supply, :photo_url, :description)
        """, new_item)
