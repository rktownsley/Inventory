...
def add_item():
    if request.method == 'POST':
        ...
        query_db("""
            INSERT INTO items 
            (name, quantity, location, category, description, instrument_type, expiration_date, 
             supplier, dosage, form, lot_number, ndc, dispense_used, unit_quantity, prescription_type)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, [name, quantity, location, category, description, instrument_type, expiration_date, 
              supplier, dosage, form, lot_number, ndc, dispense_used, unit_quantity, prescription_type])
...
def view_item(item_id):
    ...
    if request.method == 'POST':
        ...
        query_db("""
            ...
            instrument_type = ?, expiration_date = ?, supplier = ?, dosage = ?, form = ?, 
            lot_number = ?, ndc = ?, dispense_used = ?, unit_quantity = ?, prescription_type = ? 
            WHERE id = ?
        """, [name, quantity, location, category, description, instrument_type, expiration_date, 
              supplier, dosage, form, lot_number, ndc, dispense_used, unit_quantity, prescription_type, item_id])
